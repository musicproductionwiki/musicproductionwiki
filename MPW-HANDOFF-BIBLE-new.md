...
# SESSION 79 UPDATE — BIBLE — May 28, 2026

## CRITICAL ARCHITECTURE CHANGES — READ BEFORE BUILDING WRITER

### Word Count Targets — CORRECTED (Session 79)

**Old spec was wrong. Corrected from live compression.html audit:**

| Metric | Old Spec | Corrected |
|---|---|---|
| Pass 2 prose target | 4,800–5,500 words | 12,000–14,000 words |
| Pass 2 token budget | 22,000 | 32,000 per call |
| Gate 3 assembled minimum | ~4,800 words | 15,000 words |
| Gate 3 assembled target | unspecified | 17,000+ words |

compression.html prose sections = 13,684 words across 26 sections.
compression.html total assembled = 17,823 prose words (scripts/styles stripped).

### Pass 2 Split — MANDATORY (Session 79)

Pass 2 is now two separate API calls, not one:

**Pass 2A — Foundation (6,000–7,000 words):**
Sections: definition, how-it-works, new-producers, parameters, signal-chain, history
Receives: Pass 1 JSON + locked central insight + quotes_context
Does NOT receive: track lists, producer DNA, genre data

**Pass 2B — Evidence (6,500–7,500 words):**
Sections: in-the-wild, producer-dna, mix-translation, how-to-use, signatures, types, mistakes, progression, flags, faq
Receives: Pass 2A output + locked tracks (6 per slug) + locked producer DNA + genre data + Pass 1 JSON + quotes_context
Does NOT generate tracks or producer chains — writes analysis around locked data

**Rationale:** Single-call context collapse degrades in-the-wild (1,399 words) and producer-dna (932 words) which appear late in a long generation. These sections require the most specificity. Splitting prevents this.

### Four-Pass Architecture (replaces three-pass)

```
Pass 1    → Discovery + Insight Validation (API)
Pass 1.5  → Quotes Filter (no API)
Pass 2A   → Foundation Prose (API, 32K tokens, 600s timeout)
Pass 2B   → Evidence Prose (API, 32K tokens, 900s timeout)
Pass 3    → Python Assembly (no API)
```

### Track Count — LOCKED (Session 79)
6 verified tracks per slug. Not 8. Curated in `flagship_tracks.json`. Never generated.

### Curation Files — Complete List

All 9 must exist before writer runs:

| File | Status |
|---|---|
| `flagship_central_insights.json` | COMPLETE — all 40 insights locked in MPW-FLAGSHIP-WRITER-BLUEPRINT.md |
| `flagship_tool_map.json` | COMPLETE — all 40 slug assignments locked in MPW-TOOL-SESSION-PLAN.md |
| `flagship_tracks.json` | TO BUILD — 6 tracks × 40 slugs |
| `flagship_genre_data.json` | TO BUILD — genre table rows per slug |
| `flagship_producer_dna.json` | TO BUILD — 3 producers per slug |
| `flagship_plugin_recs.json` | TO BUILD — free/mid/pro per slug |
| `flagship_misconceptions.json` | TO BUILD — 3 beginner traps per slug |
| `flagship_wtrn.json` | TO BUILD — 6 WTRN cards per slug (verified slugs) |
| `quotes_merged_v2.json` | IN PROGRESS — 4 gaps + 1 resonance quote |

### Quote Gaps — Must Fix Before Writer Runs

| Slug | Status |
|---|---|
| sidechain-compression | 0 quotes ❌ |
| mix-translation | 0 quotes ❌ |
| sidechain-ducking | 0 quotes ❌ |
| reference-mixing | 0 quotes ❌ |
| resonance | 2 quotes ⚠️ (need 1 more) |

### Gate System — Extended (Session 79)

**Gate 1** — Template integrity from live compression.html (before any API calls)
**Gate 2** — Insight quality validation against flagship_central_insights.json
**Gate 3** — Post-assembly: 15,000 word minimum, 8 share bars, 5 schema blocks, zero placeholder strings, per-section word floors

Per-section word floors (Gate 3 hard fail):
- definition ≥ 800w, in-the-wild ≥ 900w, mix-translation ≥ 800w
- history ≥ 700w, how-to-use ≥ 600w, producer-dna ≥ 600w, parameters ≥ 500w

**Diff report** runs after Gate 3 passes — informational, not a gate.

### Parallel Execution Plan — Updated

```
Session A: eq, gain-staging, delay, limiting, saturation
Session B: sidechain-compression, lufs, mastering
...
Run A+B+C+D simultaneously first. Review. Then E+F+G+H simultaneously.

### New Never Rules — Session 79 (Bible Writer)

| Rule | Detail |
|---|---|
| Never target Pass 2 prose at 4,800 words | Correct target: 12,000–14,000 words |
| Never run Pass 2 as single call | Always split: Pass 2A (foundation) + Pass 2B (evidence) |
| Never generate track lists in any Pass | Use flagship_tracks.json — 6 locked tracks per slug |
| Never generate genre table numbers | Use flagship_genre_data.json |
| Never generate producer signal chains | Use flagship_producer_dna.json |
| Never run writer before all 9 curation files complete | Missing files force generation of content they would lock |
| Never run writer before Gate 1 confirms template integrity | Templates from live file every run |
| Never accept Gate 3 failure | Failed entries go to failed/ directory. No exceptions. |

## Full Reference
See `MPW-FLAGSHIP-WRITER-BLUEPRINT.md` for complete spec including all 40 pre-approved insights, full gate system, per-section word floors, pass architecture, and institutional strategy.
