...
# SESSION 79 UPDATE — CORE — May 28, 2026

## Session Type: PLANNING + ARCHITECTURE

## Priority Queue — Updated Session 79

| Priority | Task | Status |
|----------|------|--------|
| **P0** | **Session 80 — Tool Hub Redesign + Loudness Penalty build** | NEXT |
| **P0** | **Affiliate applications** — Plugin Boutique, Amazon Associates, Loopmasters, Sweetwater, PluginFox | **REVENUE BLOCKER** |
| **P0** | **GSC indexing requests** — all 42 tools + reverb + compression | Steve action |
| **P1** | Replace quotes.json with quotes_merged_v2.json in mpw-scripts\ | Steve action |
| **P1** | Save mpw_bible_writer_06.py to mpw-scripts\ | Steve action |
| **P1** | Add 1 resonance quote + fix 4 zero-coverage gaps | Steve action |
| **P1** | Decision: ClearCheck attorney listing fee pricing | Steve decision |
| **P2** | Google Workspace domain dispute Case #70817574 | Open |

## Tool Hub Architecture — LOCKED Session 79

### One Hub. One URL. Forever.
`/tools/index.html` is the single tool hub. Permanent.
`/bible/categories/tools/` is a Bible category filter only — not a hub. Links to `/tools/`.

### Hub Three-Zone Architecture (Target State)
- **Zone 1 — Hero:** Live flagship tool running on page load (Loudness Penalty Calculator)
- **Zone 2 — Flagship Cards:** 6–8 destination tools as premium cards with mini SVG preview
- **Zone 3 — Full Suite:** Category-filtered grid, all tools, improved search

### Tool Monetization Architecture (LOCKED)
- Never gate the tool itself — gate download/export output only
- Free forever: all calculation and visualization tools
- Email gate (Beehiiv): export outputs — PNG, PDF, spec cards
- Paid one-time $9–$29: comprehensive reference materials
- Bible Complete $9/mo or $79/yr: full Bible + all tools
- ClearCheck Layer 2 $29: full clearance report + letter template
- ClearCheck Layer 3: US attorney listing fee model (attorneys pay monthly to be listed)

### ClearCheck Architecture (UPDATED Session 79)
TruClarify dependency removed. ClearCheck operates as independent tool:
- Layer 1: Free (email gate) — risk score + plain-English explanation
- Layer 2: $29 one-time — full clearance intelligence report + letter template
- Layer 3: US attorney directory — attorneys pay monthly listing fee, producer chooses attorney
- Attorney directory at `/tools/clearcheck/attorneys` — filterable by state and specialization
...
### Parallel Tool Build Architecture (LOCKED Session 79)
Steve runs simultaneous Claude sessions (Max plan). Each session receives one complete self-contained build prompt file. No cross-session dependencies.

Session prompt files location: `C:\Users\swarn\OneDrive\Desktop\mpw-scripts\tool-build-prompts\`

Build groups by technology:
- Group A (Pure JS/SVG): Loudness Penalty, Dynamic Range Analyzer, Transient Shaper, Sidechain Ducking Calc, Reference Loudness Matcher, Noise Gate Visualizer, Automation Curve Visualizer, BPM DNA, Frequency Conflict Detector v2
- Group B (Web Audio API): 808 Relationship Analyzer, Chorus Explorer, Oscillator Waveform Analyzer
- Group C (Advanced/D3/Canvas): Arrangement Blueprint, Mix Fingerprint, Filter Visualizer, FM Operator, Wavetable Visualizer, Modulation Matrix, Mix Translation Simulator
- Group D (Claude API): Vocal Chain Builder, Reference Decoder, Stem Quality Tester, ClearCheck

## New Document Files — Session 79

| File | Purpose | Location |
|---|---|---|
| `MPW-TOOL-SESSION-PLAN.md` | Complete tool hub strategy, all 12 tool specs, parallel build architecture | GitHub + mpw-scripts\ |
| `MPW-FLAGSHIP-WRITER-BLUEPRINT.md` | Complete Bible writer spec, all 40 insights, 4-pass architecture, gate system | GitHub + mpw-scripts\ |

Both files are authoritative. They supersede all prior tool and writer planning notes in all handoffs.

## Session Sequence — Sessions 80+

```
Session 80:       Hub redesign + Loudness Penalty + session prompt files for all tools
Sessions 81-84:   Parallel tool builds (4 groups, multiple simultaneous sessions per group)
Sessions 85-86:   Curation files build (insights locked, tracks/DNA/genre/plugins/WTRN)
Session 87:       Bible flagship writer build + test on eq
Sessions 88+:     Bible writer parallel batch runs (8 simultaneous sessions)
```

## Gold Standard — Unchanged
- **Structure + SEO:** `bible/compression.html` v1.2 — SHA `9de422e2`
- **Prose + depth:** `bible/reverb.html` v1.6
- Both must be studied before writing any new flagship entry

## New Never Rules — Session 79

| Rule | Detail |
|------|--------|
| Never gate the tool itself — gate the download/export output only | Locked monetization rule — applies to every tool on the site |
| Never build a tool without its monetization hook coded from day one | Email gate on export, not retrofitted later |
