...
## SESSION 79 NEW NEVER RULES

### CATEGORY: BIBLE WRITER

| Rule | Session | Reason | Enforcement |
|---|---|---|---|
| Never target Pass 2 prose at 4,800 words — correct target is 12,000–14,000 words | S79 | compression.html prose sections total 13,684 words. Old spec measured wrong metric. | MANUAL |
| Never run Pass 2 as a single API call — always split into Pass 2A (foundation) and Pass 2B (evidence) | S79 | Single-call context collapse degrades in-the-wild and producer-dna sections which appear late in generation. These require the most specificity. | MANUAL |
| Never generate track lists in any Pass — use flagship_tracks.json (6 locked tracks per slug) | S79 | Fabricated track analyses are the highest-risk content in any flagship entry. Named tracks with timestamps must be verified. | MANUAL |
| Never generate genre table numbers in any Pass — use flagship_genre_data.json | S79 | Generated numbers sound plausible but may be wrong. Curated data only. | MANUAL |
| Never generate producer signal chains in any Pass — use flagship_producer_dna.json | S79 | Fabricated signal chains undermine institutional authority standard. | MANUAL |
| Never run the flagship writer before all 9 curation files are complete and in mpw-scripts\ | S79 | Missing curation files force generation of content they would have locked — defeats the entire curation architecture. | MANUAL |
| Never accept Gate 3 failure as acceptable — all failed entries go to failed/ directory with report | S79 | Every committed flagship entry has passed all gates. No partial passes. No "close enough." | MANUAL |

### CATEGORY: TOOLS

| Rule | Session | Reason | Enforcement |
|---|---|---|---|
| Never gate the tool itself — gate the download/export output only | S79 | Locked monetization rule. Gating the tool kills usage, backlinks, and SEO. Gating the download captures email without friction. | MANUAL |
| Never build a tool without its monetization hook (email gate on download) coded from day one | S79 | Retrofitting monetization after build is technically messier and frequently skipped. Build it in. | MANUAL |
| Never build a parallel session tool without a complete self-contained session prompt file | S79 | Each build session is self-contained. Partial prompts produce partial tools. | MANUAL |
| Never commit a tool page without updating /tools/index.html hub card in same Trees API commit | S79 | Hub and tool page are one atomic unit. Hub going stale is a user experience failure. | MANUAL |
| Never reference TruClarify in ClearCheck architecture — ClearCheck is fully independent | S79 | ClearCheck Layer 3 is US attorney listing fee model. No TruClarify dependency. | MANUAL |

## MASTER COUNTS (Session 79)
Total rules documented: 146
SCRIPT-ENFORCED: 17
MANUAL: 129
