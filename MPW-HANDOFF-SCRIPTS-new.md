...
# SESSION 79 UPDATE — SCRIPTS — May 28, 2026

## New Script Files — Session 79

| File | Location | Purpose |
|---|---|---|
| `MPW-TOOL-SESSION-PLAN.md` | `mpw-scripts\` + GitHub | Complete tool hub plan, all 12 tool specs, parallel build architecture |
| `MPW-FLAGSHIP-WRITER-BLUEPRINT.md` | `mpw-scripts\` + GitHub | Complete Bible writer spec, 40 insights, 4-pass architecture |
| `flagship_tool_map.json` | `mpw-scripts\` | Locked tool assignment per slug — all 40 flagship entries |
| `flagship_central_insights.json` | `mpw-scripts\` | All 40 pre-approved central insights — locked |

## mpw_flagship_writer.py — Architecture Update (Session 79)

### Four-Pass Architecture (replaces three-pass)

```python
mpw_flagship_writer.py
├── load_curation_files()         # 9 JSON files — fail loudly if any missing
├── gate1_template_integrity()    # Fetch compression.html, validate fingerprints
├── pass1_discovery()             # API, 20K tokens, 300s — JSON output
├── gate2_insight_quality()       # Validate vs flagship_central_insights.json
├── pass1_5_quotes()              # No API — filter quotes_merged_v2.json
├── pass2a_foundation()           # API, 32K tokens, 600s — prose HTML (conceptual)
├── pass2b_evidence()             # API, 32K tokens, 900s — prose HTML (evidence)
├── pass3_assemble()              # Python only — full HTML
├── gate3_validation()            # 15K word min, 8 share bars, 5 schema, zero placeholders
├── diff_report()                 # Structural compare vs compression.html
├── commit()                      # Trees API
└── run_batch()                   # ThreadPoolExecutor parallel
```

### Pass 2A — Foundation (new)
Target: 6,000–7,000 prose words
Sections: definition, how-it-works, new-producers, parameters, signal-chain, history
Tokens: 32,000 | Timeout: 600s
Input: Pass 1 JSON + central insight + quotes_context
NOT input: track lists, producer DNA, genre data

### Pass 2B — Evidence (new)
Target: 6,500–7,500 prose words
Sections: in-the-wild, producer-dna, mix-translation, how-to-use, signatures, types, mistakes, progression, flags, faq
Tokens: 32,000 | Timeout: 900s
Input: Pass 2A output + locked tracks + locked producer DNA + genre data + Pass 1 JSON + quotes_context
CRITICAL: Does not generate tracks or chains — writes analysis around locked data

### API Cost per Entry (updated)
~84,000 tokens total = ~$0.40–0.50 per flagship entry
40 entries = ~$16–20 total

### Delivery Format (unchanged)
3-part PS1 install scripts under 200KB each:
- `install_flagship_writer_part1.ps1`
- `install_flagship_writer_part2.ps1`
- `install_flagship_writer_part3.ps1`

Final part smoke test: generates eq.html locally, prints word count + validation score + first paragraph.

## Tool Build Session Prompt Files

Location: `C:\Users\swarn\OneDrive\Desktop\mpw-scripts\tool-build-prompts\`

One complete self-contained `.md` file per tool. Steve hands one file to each parallel Claude session.

Required contents per session prompt file:
- Full design system CSS variables (copy verbatim)
- All tool-specific never rules
- Complete tool spec (inputs, outputs, tech, presets, monetization hook)
- Frozen nav HTML (exact from live site)
- Share bar pattern (exact from compression.html)
- Embed code pattern
- Validation checklist
- Commit instruction (Trees API — tool page + hub card update in same commit)

Session prompt files are written in Session 80 and committed to GitHub for reuse.

## mpw_precommit_check.py (documentation update)

**Location:** `C:\Users\swarn\OneDrive\Desktop\mpw-scripts\mpw_precommit_check.py`
**Run:** `python mpw_precommit_check.py [filename]`
**When:** Before every single commit. No exceptions.

What it catches:
- Raw GitHub tokens in files
- Missing SEO elements (canonical, og tags, twitter:card)
- Unreplaced placeholder strings
- innerHTML usage in JS (CSP violation)
- style.css references in tool pages (blob shapes)
- .hero or .container class in tool pages (global CSS conflict)
- Missing share bars
- Missing embed code blocks
- Missing GA4 tag

## Wave Batch Files (updated — 8 waves)

```
flagships_wave1a.txt  → eq, gain-staging, delay, limiting, saturation
flagships_wave1b.txt  → sidechain-compression, lufs, mastering
flagships_wave2a.txt  → parallel-compression, bus-compression, stereo-imaging, mid-side-processing, automation
flagships_wave2b.txt  → high-pass-filter, parametric-eq, multiband-compression, noise-gate, dynamic-range
flagships_wave2c.txt  → headroom, subtractive-synthesis, lfo, adsr, mix-translation
flagships_wave3a.txt  → transient-shaping, fm-synthesis, wavetable-synthesis, oscillator, true-peak-limiting
flagships_wave3b.txt  → loudness-normalization, send-return, harmonic-distortion, resonance, sidechain-ducking
flagships_wave3c.txt  → modulation, low-pass-filter, arrangement, reference-mixing
```
